...
<head>
    ...
    <style>
        ...
        .hero-card { border: 0; border-radius: 20px; box-shadow: 0 12px 30px rgba(15, 23, 42, 0.08); }
        ...
        .event-box { font-size: 12px; line-height: 1.15; }
        .event-box strong { display: block; }
    </style>
</head>
<body>
                        ...
                        @forelse($citasHoy as $cita)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <div class="text-muted small">{{ $cita->fec_i?->format('H:i') }} - {{ $cita->fec_f?->format('H:i') }}</div>
                                </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventContent: function (arg) {
                    const doctor = arg.event.extendedProps.doctor ? `<div class="small">${arg.event.extendedProps.doctor}</div>` : '';
                    return { html: `<div class="event-box"><strong>${arg.event.title}</strong><div>${arg.timeText}</div>${doctor}<div>${arg.event.extendedProps.estado || ''}</div></div>` };
                }
            });
